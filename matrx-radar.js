(function (global, factory) {
	...
	function create_main_fragment(component, ctx) {
		var g, defs, text, textPath, text_1, textPath_xlink_href_value, path, path_d_value;
	...
	function levelConfigAnnotated({levelConfig, baseColor}) {
	  ...
	  for (let level of levelConfig) {
	    ...
	    level.labelID = getID$1();
	    level.descriptionID = getID$1();
	...
	function create_main_fragment$4(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				if (changed.disciplinesAnnotated || changed.disciplineFontSize || changed.practiceFontSize || changed.goalFontSize || changed.centerX || changed.centerY || changed.outerRadius || changed.innerRadius || changed.strokeWidth || changed.disciplineStroke || changed.disciplineBandHeight || changed.diciplineFontColor || changed.goalColor || changed.practiceBandHeight || changed.practiceStroke || changed.fontColor || changed.levelConfigAnnotated) {
	...
	function create_each_block$2(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.disciplinesAnnotated || changed.practiceFontSize || changed.goalFontSize || changed.centerX || changed.centerY || changed.outerRadius || changed.innerRadius || changed.strokeWidth || changed.disciplineStroke || changed.disciplineBandHeight || changed.diciplineFontColor || changed.disciplineFontSize || changed.goalColor || changed.practiceBandHeight || changed.practiceStroke || changed.fontColor || changed.levelConfigAnnotated) {
	...
	function create_each_block_1(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.outerRadius) arc_changes.outerRadius = ctx.outerRadius;
				if (changed.innerRadius) arc_changes.innerRadius = ctx.innerRadius;
				...
				if (changed.outerRadius || changed.disciplineBandHeight) arc_1_changes.innerRadius = ctx.outerRadius-ctx.disciplineBandHeight;
				if (changed.outerRadius) arc_1_changes.outerRadius = ctx.outerRadius;
				if (changed.disciplineBandHeight) arc_1_changes.labelBandHeight = ctx.disciplineBandHeight;
				...
				if (changed.disciplinesAnnotated) arc_1_changes.label = ctx.discipline.label;
				if (changed.disciplinesAnnotated) arc_1_changes.description = ctx.discipline.description;
				if (changed.diciplineFontColor) arc_1_changes.fontColor = ctx.diciplineFontColor;
				if (changed.disciplineFontSize) arc_1_changes.fontSize = ctx.disciplineFontSize;
				arc_1._set(arc_1_changes);
	...
	function Radar(options) {
		...
		this._recompute({ disciplines: 1, practiceCount: 1, outerRadius: 1, disciplineBandHeight: 1, sliceWidth: 1, practiceBandHeight: 1, practiceMaxWidth: 1, practiceMaxHeight: 1, levelConfig: 1, baseColor: 1, goalColor: 1, levelConfigAnnotated: 1 }, this._state);
		this._intro = true;
