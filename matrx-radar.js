(function (global, factory) {
	...
	function junk(s) {
	  console.log(s.pointerEvents);
	}

	function pointerEvents({description}) {
		return description ? "all" : "none";
	}
	...
	function Arc(options) {
		...
		this._recompute({ description: 1, innerRadius: 1, centerX: 1, startRadians: 1, centerY: 1, endRadians: 1, outerRadius: 1, pMRadius: 1 }, this._state);
		this._intro = true;
	...
	function create_main_fragment$2(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.dataOuterRadius) arc_changes.outerRadius = ctx.dataOuterRadius;
				...
				if (changed.description) arc_1_changes.description = ctx.description;
				if (changed.dataOuterRadius) arc_1_changes.innerRadius = ctx.dataOuterRadius;
				if (changed.outerRadius) arc_1_changes.outerRadius = ctx.outerRadius;
				if (changed.label) arc_1_changes.label = ctx.label;
				if (changed.fontColor) arc_1_changes.fontColor = ctx.fontColor;
				if (changed.fontSize) arc_1_changes.fontSize = ctx.fontSize;
	...
	// (4:2) {#each levelsAnnotated as level}
	...
	function create_each_block$2(component, ctx) {
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.disciplinesAnnotated) arc_changes.description = ctx.discipline.description;
