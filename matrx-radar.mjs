...
function create_main_fragment$3(component, ctx) {
	...
	return {
		...
		p(changed, ctx) {
			if (changed.margin || changed.startY || changed.scale || changed.levelConfigAnnotated) {
...
function create_each_block$1(component, ctx) {
	var rect, rect_x_value, rect_y_value, rect_height_value, rect_width_value, rect_fill_value, rect_opacity_value, text, tspan, text_1_value = ctx.level.label, text_1, text_id_value, text_x_value, text_y_value;
	...
	return {
		c() {
			...
			text_1 = createText(text_1_value);
			setAttribute(rect, "x", rect_x_value = 105+ctx.margin);
			setAttribute(rect, "y", rect_y_value = ctx.startY+ctx.i*2*ctx.scale+2*ctx.margin);
			setAttribute(rect, "height", rect_height_value = ctx.scale-2*ctx.margin);
			setAttribute(rect, "width", rect_width_value = ctx.scale-2*ctx.margin);
			setAttribute(rect, "fill", rect_fill_value = ctx.level.color);
			setAttribute(rect, "opacity", rect_opacity_value = ctx.level.opacity);
			setAttribute(text, "id", text_id_value = ctx.level.id);
			setAttribute(text, "x", text_x_value = 105+2*ctx.scale);
			setAttribute(text, "y", text_y_value = ctx.startY+ctx.i*2*ctx.scale+ctx.scale);
			setAttribute(text, "font-size", ctx.scale);
			setAttribute(text, "alignment-baseline", "middle");
		...
		m(target, anchor) {
			...
			insertNode(text, target, anchor);
			appendNode(tspan, text);
			appendNode(text_1, tspan);
		},

		p(changed, ctx) {
			if ((changed.margin) && rect_x_value !== (rect_x_value = 105+ctx.margin)) {
				setAttribute(rect, "x", rect_x_value);
			}

			if ((changed.startY || changed.scale || changed.margin) && rect_y_value !== (rect_y_value = ctx.startY+ctx.i*2*ctx.scale+2*ctx.margin)) {
				setAttribute(rect, "y", rect_y_value);
			}

			if ((changed.scale || changed.margin) && rect_height_value !== (rect_height_value = ctx.scale-2*ctx.margin)) {
				setAttribute(rect, "height", rect_height_value);
			}

			if ((changed.scale || changed.margin) && rect_width_value !== (rect_width_value = ctx.scale-2*ctx.margin)) {
			...
			if ((changed.levelConfigAnnotated) && rect_opacity_value !== (rect_opacity_value = ctx.level.opacity)) {
				setAttribute(rect, "opacity", rect_opacity_value);
			}
			...
			if ((changed.levelConfigAnnotated) && text_id_value !== (text_id_value = ctx.level.id)) {
				setAttribute(text, "id", text_id_value);
			}

			if ((changed.scale) && text_x_value !== (text_x_value = 105+2*ctx.scale)) {
...
function Legend(options) {
	...
	this._recompute({ scale: 1, levelConfigAnnotated: 1 }, this._state);
	this._intro = true;
...
function create_main_fragment$4(component, ctx) {
	...
	var legend_initial_data = { levelConfigAnnotated: ctx.levelConfigAnnotated };
