...
function labelArcEnds({goalAnnotated, centerX, centerY, startRadians, endRadians, scale}) {
  return getArcEnds(goalAnnotated.radius - scale, centerX, centerY, startRadians, endRadians)
}
...
let getID = () =>
 ...
 function disciplinesAnnotated({disciplines, outerRadius, disciplineBandHeight}) {
  ...
  let disciplineMaxWidth = 0.9 * (outerRadius - disciplineBandHeight / 2);
  console.log(disciplineMaxWidth);
...
function create_main_fragment$4(component, ctx) {
	...
	return {
		...
		p(changed, ctx) {
			if (changed.disciplinesAnnotated || changed.centerX || changed.centerY || changed.innerRadius || changed.outerRadius || changed.disciplineBandHeight || changed.practiceBandHeight || changed.practiceStroke || changed.strokeWidth || changed.fontColor || changed.practiceFontSize || changed.levelConfigAnnotated || changed.disciplineStroke || changed.diciplineFontColor || changed.disciplineFontSize) {
...
function create_each_block$2(component, ctx) {
	...
	return {
		...
		p(changed, ctx) {
			if (changed.centerX || changed.centerY || changed.disciplinesAnnotated || changed.innerRadius || changed.outerRadius || changed.disciplineBandHeight || changed.practiceBandHeight || changed.practiceStroke || changed.strokeWidth || changed.fontColor || changed.practiceFontSize || changed.levelConfigAnnotated) {
...
function Radar(options) {
	...
	this._recompute({ disciplines: 1, outerRadius: 1, disciplineBandHeight: 1, levelConfig: 1, baseColor: 1 }, this._state);
	this._intro = true;
...
Radar.prototype._recompute = function _recompute(changed, state) {
	if (changed.disciplines || changed.outerRadius || changed.disciplineBandHeight) {
